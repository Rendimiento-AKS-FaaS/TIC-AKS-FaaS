...
require "media_service_MovieInfoService"
...
-- req_id: derivado del request_id de Nginx (truncado a 15 hex chars → i64)
local req_id = tonumber(string.sub(ngx.var.request_id or "0000000000000000", 1, 15), 16) or 0
local carrier = {}

-- Obtener cliente Thrift del pool
local client = GenericObjectPool:connection(
    MovieInfoServiceClient,
    "movie-info-service.default.svc.cluster.local",
    9090
)
...
if not ok then
    ...
    ngx.say(cjson.encode({ error = "MovieInfoService error: " .. err_msg }))
    return
end
