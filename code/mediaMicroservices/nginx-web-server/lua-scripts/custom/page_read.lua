...
require "media_service_PageService"
...
local carrier      = {}
local review_start = tonumber(args.review_start or 0)
local review_stop  = tonumber(args.review_stop  or 10)

-- PageServiceClient es una global definida en media_service_PageService.lua
local client = GenericObjectPool:connection(
    PageServiceClient,
    "page-service.default.svc.cluster.local",
    9090
)
...
if not ok then
    ...
    ngx.say(cjson.encode({ error = "PageService error: " .. err_msg }))
    return
end

-- Serializar resultado Thrift a tabla Lua plana (evita errores de cjson con metatables)
